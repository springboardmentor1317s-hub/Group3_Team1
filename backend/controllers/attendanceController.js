...
const EventCertificateTemplate = require("../models/EventCertificateTemplate");
...
function createCertificateNumber(eventId, studentId) {
  const digest = crypto
    .createHash("sha1")
    .update(`${String(eventId || "")}|${String(studentId || "")}|certificate`)
    .digest("hex")
    .toUpperCase();

  return `CEH-CERT-${digest.slice(0, 4)}-${digest.slice(4, 8)}-${digest.slice(8, 12)}`;
}
...
async function resolveCertificateBrandAssets(templateDoc) {
  const logoPathCandidates = [
    path.resolve(__dirname, "../../public/favicon-ceh.png"),
    path.resolve(__dirname, "../../public/icon2.png")
  ];
  ...
  const signatureBuffer = await loadImageBufferFromUrl(templateDoc?.signatureDataUrl);

  return { logoBuffer, signatureBuffer };
}

async function buildCertificatePdfBuffer({
  event,
  registration,
  student,
  certificate,
  ...
  const outerX = 28;
  const outerY = 28;
  const outerW = pageWidth - 56;
  const outerH = pageHeight - 56;
  const innerX = outerX + 12;
  const innerY = outerY + 12;
  const innerW = outerW - 24;
  const innerH = outerH - 24;

  doc.save();
  const background = doc.linearGradient(0, 0, pageWidth, pageHeight);
  background.stop(0, "#fffdf7").stop(0.48, "#f7fbff").stop(1, "#fff7ed");
  doc.rect(0, 0, pageWidth, pageHeight).fill(background);
  doc.restore();

  doc.roundedRect(outerX, outerY, outerW, outerH, 26).lineWidth(2.5).stroke("#d4a017");
  doc.roundedRect(innerX, innerY, innerW, innerH, 20).lineWidth(1).stroke("#bfdbfe");

  const { logoBuffer, signatureBuffer } = await resolveCertificateBrandAssets(templateDoc);
  if (logoBuffer) {
    doc.image(logoBuffer, outerX + 28, outerY + 24, { fit: [82, 82], align: "center", valign: "center" });
  }

  doc.font("Helvetica-Bold").fillColor("#1e3a8a").fontSize(18).text("Campus Event Hub", 0, outerY + 32, {
    align: "center"
  });
  doc.font("Helvetica").fillColor("#475569").fontSize(11).text("Certificate of Participation", 0, outerY + 56, {
    align: "center"
  });

  doc.font("Helvetica-Bold").fillColor("#b45309").fontSize(28).text("CERTIFICATE", 0, outerY + 98, {
    align: "center"
  });
  doc.font("Helvetica-Bold").fillColor("#0f172a").fontSize(15).text("OF PARTICIPATION", 0, outerY + 132, {
    align: "center"
  });

  doc.font("Helvetica").fillColor("#475569").fontSize(15).text("This certificate is proudly presented to", 0, outerY + 186, {
    align: "center"
  });

  const studentName = wrapText(student?.name || registration?.studentName || "Student");
  doc.font("Helvetica-Bold").fillColor("#111827").fontSize(30).text(studentName, 0, outerY + 220, {
    align: "center"
  });

  doc.moveTo(pageWidth * 0.25, outerY + 265).lineTo(pageWidth * 0.75, outerY + 265).lineWidth(1).stroke("#cbd5e1");

  const eventName = wrapText(event?.name || registration?.eventName || "Campus Event");
  const organizationName = wrapText(templateDoc?.organizationName || "Campus Event Hub");
  const locationText = wrapText(event?.location || registration?.college);
  const dateText = formatDateTime(event?.dateTime || certificate?.generatedAt);

  const paragraph = `for successfully participating in ${eventName} organized by ${organizationName} on ${dateText} at ${locationText}.`;
  doc.font("Helvetica").fillColor("#334155").fontSize(15).text(paragraph, outerX + 90, outerY + 292, {
    width: outerW - 180,
    align: "center",
    lineGap: 4
  });

  doc.font("Helvetica").fillColor("#64748b").fontSize(11).text(`Certificate No: ${wrapText(certificate?.certificateNumber)}`, outerX + 28, outerY + outerH - 78);
  doc.font("Helvetica").fillColor("#64748b").fontSize(11).text(`Issued On: ${formatDateTime(certificate?.generatedAt || new Date())}`, outerX + 28, outerY + outerH - 58);

  const signatureBaseY = outerY + outerH - 122;
  const signatureBaseX = outerX + outerW - 220;
  if (signatureBuffer) {
    doc.image(signatureBuffer, signatureBaseX, signatureBaseY - 26, {
      fit: [150, 54],
      align: "center",
      valign: "center"
    });
  }
  doc.moveTo(signatureBaseX, signatureBaseY + 34).lineTo(signatureBaseX + 150, signatureBaseY + 34).lineWidth(1).stroke("#0f172a");
  doc.font("Helvetica-Bold").fillColor("#0f172a").fontSize(12).text(wrapText(templateDoc?.signerName, organizationName), signatureBaseX, signatureBaseY + 40, {
    width: 150,
    align: "center"
  });
  doc.font("Helvetica").fillColor("#64748b").fontSize(10).text(wrapText(templateDoc?.signerTitle, "Authorized Signature"), signatureBaseX, signatureBaseY + 56, {
    width: 150,
    align: "center"
  });

  doc.font("Helvetica").fillColor("#64748b").fontSize(9).text("Generated by Campus Event Hub", 0, pageHeight - 34, {
    align: "center"
  });
  ...
  try {
    ...
    const items = approvedRegistrations.map((registration) => {
      ...
      return {
        ...
        canDownloadAdmitCard: !!card?.distributedAt
      };
    });
...
exports.distributeAdmitCardsForEvent = async (req, res) => {
  try {
    if (!isAdminRole(req.user?.role)) {
      return res.status(403).json({ message: "Only admins can distribute admit cards." });
    }
    ...
    const generatedCards = await AdmitCard.find({ eventId });
    if (!generatedCards.length) {
      return res.status(400).json({ message: "Please generate admit cards before distributing them." });
    }

    const distributedAt = new Date();
    const distributedBy = String(req.user?.id || "");
    ...
    return res.json({
      message: "Admit cards distributed successfully.",
      distributed: Number(result.modifiedCount || 0),
      total: generatedCards.length
    });
  } catch (error) {
    return res.status(500).json({
      message: "Failed to distribute admit cards.",
      details: String(error?.message || "")
    });
  }
};
...
exports.downloadMyAdmitCard = async (req, res) => {
  try {
    ...
    if (!adminPreviewMode && !admitCard.distributedAt) {
      return res.status(403).json({
        message: "Admit card is generated but not distributed by admin yet."
      });
    }
...
exports.getEventAttendanceRoster = async (req, res) => {
  try {
    ...
    const [event, registrations, attendanceRecords, admitCards, certificates, certificateTemplate] = await Promise.all([
      Event.findById(eventId),
      Registration.find({
        eventId,
        status: { $regex: /^APPROVED$/i }
      ...
      EventCertificate.find({ eventId }),
      EventCertificateTemplate.findOne({ eventId }).lean()
    ]);
    ...
    const certificateMap = new Map(certificates.map((item) => [String(item.studentId), item]));
    ...
    const students = registrations.map((registration) => {
      ...
      const certificate = certificateMap.get(String(registration.studentId));
      return {
        registrationId: String(registration._id),
        studentId: registration.studentId,
        ...
        certificateIssuedAt: certificate?.generatedAt || null,
    ...
    return res.json({
      ...
      certificateTemplate: {
        hasSignature: Boolean(certificateTemplate?.signatureDataUrl),
        organizationName: wrapText(certificateTemplate?.organizationName, "Campus Event Hub"),
        signerName: wrapText(certificateTemplate?.signerName, ""),
        signerTitle: wrapText(certificateTemplate?.signerTitle, "")
      },
      students
    });
  } catch (error) {
...
exports.saveCertificateTemplateForEvent = async (req, res) => {
  try {
    if (!isAdminRole(req.user?.role)) {
      return res.status(403).json({ message: "Only admins can manage certificate templates." });
    }
    ...
    const hasAccess = await hasAdminEventAccess({
      userId: req.user?.id,
      role: req.user?.role,
      eventId,
      eventDoc: event
    ...
    if (!hasAccess) {
      return res.status(403).json({ message: "You are not allowed to manage this event certificate." });
    }

    const existingTemplate = await EventCertificateTemplate.findOne({ eventId });
    const uploadedSignatureDataUrl = normalizeDataUrlImage(req.body?.signatureDataUrl);
    const signatureDataUrl = uploadedSignatureDataUrl || String(existingTemplate?.signatureDataUrl || "");
    if (!signatureDataUrl) {
      return res.status(400).json({ message: "A valid signature image is required." });
    }

    const organizationName = String(req.body?.organizationName || "Campus Event Hub").trim() || "Campus Event Hub";
    const signerName = String(req.body?.signerName || "").trim();
    const signerTitle = String(req.body?.signerTitle || "").trim();

    const template = await EventCertificateTemplate.findOneAndUpdate(
      { eventId },
      {
        $set: {
          organizationName,
    ...
    return res.json({
      ...
      template: {
        ...
        hasSignature: Boolean(template.signatureDataUrl),
        uploadedAt: template.uploadedAt
      }
    });
  } catch (error) {
    return res.status(500).json({
      message: "Failed to save certificate signature.",
      details: String(error?.message || "")
    });
  }
};
...
exports.generateCertificatesForEvent = async (req, res) => {
  try {
    if (!isAdminRole(req.user?.role)) {
      return res.status(403).json({ message: "Only admins can generate certificates." });
    }
    ...
    if (!hasAccess) {
      return res.status(403).json({ message: "You are not allowed to generate certificates for this event." });
    }

    const template = await EventCertificateTemplate.findOne({ eventId }).lean();
    if (!template?.signatureDataUrl) {
      return res.status(400).json({ message: "Upload an authorized signature before generating certificates." });
    }
    ...
    if (!attendanceRecords.length) {
      return res.status(400).json({ message: "No attended students found for this event yet." });
    }

    const registrationMap = new Map(registrations.map((registration) => [String(registration.studentId), registration]));
    ...
    for (const attendance of attendanceRecords) {
      ...
      const registration = registrationMap.get(studentId);
      if (!registration) {
        failed += 1;
        continue;
      }

      const update = await EventCertificate.updateOne(
        { eventId, studentId },
        {
          $set: {
            registrationId: String(registration._id),
            attendanceId: String(attendance._id || ""),
            certificateNumber: createCertificateNumber(eventId, studentId),
      ...
      created += Number(update.upsertedCount || 0);
      refreshed += Number(update.matchedCount || 0);
    }
...
exports.getMyCertificateStatus = async (req, res) => {
  try {
    ...
    if (!studentId || !eventId) {
      return res.status(400).json({ message: "Invalid student or event.", canDownload: false });
    }
    ...
    const [registration, attendance, certificate] = await Promise.all([
      ...
      AttendanceRecord.findOne({ studentId, eventId }).lean(),
      EventCertificate.findOne({ studentId, eventId }).lean()
    ]);

    return res.json({
      canDownload: Boolean(registration && attendance && certificate),
...
exports.downloadMyCertificate = async (req, res) => {
  try {
    ...
    const [event, registration, attendance, certificate, student, template] = await Promise.all([
      ...
      AttendanceRecord.findOne({ studentId, eventId }).lean(),
      EventCertificate.findOne({ studentId, eventId }),
    ...
    if (!registration || !attendance || !certificate) {
      return res.status(403).json({
        message: "Certificate is available only for students who were marked present in this event."
      });
    }

    if (!template?.signatureDataUrl) {
      return res.status(400).json({ message: "Certificate template is not ready yet for this event." });
    }
    ...
    const safeEventName = String(event.name || "event").replace(/[^a-z0-9]+/gi, "_");
    const fileName = `certificate_${safeEventName}.pdf`;
    res.setHeader("Content-Type", "application/pdf");
